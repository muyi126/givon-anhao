...
public class ContactlistFragment extends Fragment {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (Constant.NEW_FRIENDS_USERNAME.equals(username)) {
					// 进入申请与通知页面
					UserBean user = AnhaoApplication.getInstance().getContactList().get(Constant.NEW_FRIENDS_USERNAME);
	...
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getItemId() == R.id.delete_contact) {
			UserBean tobeDeleteUser = adapter.getItem(((AdapterContextMenuInfo) item.getMenuInfo()).position);
			// 删除此联系人
			...
			UserBean user = adapter.getItem(((AdapterContextMenuInfo) item.getMenuInfo()).position);
			try {
				//加入到黑名单
